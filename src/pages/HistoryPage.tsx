...
const HistoryPage = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (learningPlans) {
        combinedHistory.push(...learningPlans.map(plan => ({
          id: plan.id,
      ...
      if (quizResults) {
        combinedHistory.push(...quizResults.map(quiz => ({
          id: quiz.id,
          type: 'quiz' as const,
  ...
  const getTierColor = (tier?: string) => {
    switch (tier) {
      case 'basic': return 'text-blue-400 bg-blue-400/10 border-blue-400/20';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="flex justify-center mb-6">
            <div className="p-4 rounded-full bg-gradient-to-r from-orange-400 to-red-400 animate-pulse">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="block w-full pl-10 pr-3 py-3 bg-gray-700/50 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition-all duration-300"
              />
            </div>
        ...
        {filteredHistory.length > 0 ? (
          <div className="space-y-6">
            {filteredHistory.map((item) => (
              <div key={item.id} className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-2xl p-6 hover:border-gray-600 transition-all duration-300">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4">
                      {item.tier && (
                        <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getTierColor(item.tier)}`}>
                          {item.tier.charAt(0).toUpperCase() + item.tier.slice(1)}
                        </span>
                      )}
                      
                      {item.difficulty && (
                        <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getDifficultyColor(item.difficulty)}`}>
                      ...
                      {item.score !== undefined && item.total_questions && (
                        <div className="flex items-center space-x-2">
                          ...
                          <span className={`font-semibold ${getScoreColor(item.score / item.total_questions * 100)}`}>
                            {item.score}/{item.total_questions} ({Math.round(item.score / item.total_questions * 100)}%)
                          </span>
                        </div>
                      )}
                    </div>
          ...
          <div className="text-center py-16">
            ...
            {!searchTerm && filter === 'all' && (
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a 
                  href="/learning-plans"
                  className="px-6 py-3 bg-gradient-to-r from-green-500 to-cyan-500 hover:from-green-400 hover:to-cyan-400 rounded-lg font-medium transition-all duration-300 transform hover:scale-105"
                >
                  Create Learning Plan
                </a>
                <a 
                  href="/quiz"
                  className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-400 hover:to-pink-400 rounded-lg font-medium transition-all duration-300 transform hover:scale-105"
                >
                  Take Quiz
                </a>
              </div>
                            ...
                            return (
                              <div>
                                ...
                                {(planData.schedule || planData.studySchedule) && (
                                  <div className="mb-6">
                                    <h3 className="text-xl font-bold text-white mb-4">Study Schedule</h3>
                                    <div className="grid md:grid-cols-2 gap-4">
                                      {(planData.schedule?.weekly || planData.studySchedule?.weekly) && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="text-lg font-semibold text-white mb-3">Weekly Plan</h4>
                                          <p className="text-gray-300">{planData.schedule?.weekly || planData.studySchedule?.weekly}</p>
                                        </div>
                                      )}
                                      {(planData.schedule?.daily || planData.studySchedule?.daily) && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="text-lg font-semibold text-white mb-3">Daily Recommendations</h4>
                                          <p className="text-gray-300">{planData.schedule?.daily || planData.studySchedule?.daily}</p>
                                        </div>
                                      )}
                                    </div>
                                  </div>
                                    ...
                                    <div className="space-y-4">
                                      {planData.phases.map((phase: any, index: number) => (
                                        <div key={index} className="bg-gray-700/20 rounded-lg p-4">
                                          ...
                                          {phase.objectives && (
                                            <div className="mb-3">
                                              <p className="text-sm font-medium text-gray-400 mb-2">Objectives:</p>
                                              <div className="space-y-2">
                                                {phase.objectives.map((obj: any, objIndex: number) => (
                                                  <div key={objIndex} className="bg-gray-800/50 rounded p-3">
                                                    <div className="text-sm text-gray-300">
                                                      {typeof obj === 'string' ? obj : obj.title || obj.description || `Objective ${objIndex + 1}`}
                                                    </div>
                                                    {typeof obj === 'object' && obj.timeRequired && (
                                                      <div className="text-xs text-cyan-400 mt-1">⏱️ {obj.timeRequired}</div>
                                                    )}
                                                  </div>
                                                ))}
                                              </div>
                                            ...
                                            <div>
                                              ...
                                              <div className="space-y-2">
                                                {phase.exercises.map((exercise: any, exIndex: number) => (
                                                  <div key={exIndex} className="bg-gray-800/50 rounded p-3">
                                                    <div className="text-sm font-medium text-white">
                                                    ...
                                                    {exercise.timeRequired && (
                                                      <div className="text-xs text-cyan-400 mt-1">⏱️ {exercise.timeRequired}</div>
                                                    )}
                                                  </div>
                                                ))}
                                              </div>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                      {planData.projects.map((project: any, index: number) => (
                                        <div key={index} className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="text-lg font-semibold text-white mb-2">{project.title || `Project ${index + 1}`}</h4>
                                          <p className="text-gray-300 text-sm mb-3">{project.description || 'No description available'}</p>
                                  ...
                                  <div className="mb-6">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                      {planData.toolsAndSoftware.map((tool: any, index: number) => (
                                        <div key={index} className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-2">{tool.name || tool.title || `Tool ${index + 1}`}</h4>
                                          <p className="text-gray-400 text-sm mb-2">{tool.purpose || tool.description || 'No description available'}</p>
                                          <div className="flex items-center justify-between">
                                            <span className={`inline-block px-2 py-1 rounded text-xs ${
                                              (tool.cost === 'Free' || tool.price === 'Free') ? 'bg-green-400/20 text-green-400' : 'bg-yellow-400/20 text-yellow-400'
                                            }`}>
                                ...
                                {planData.assessmentMethods && (
                                  <div className="mb-6">
                                    <h3 className="text-xl font-bold text-white mb-4">Assessment Methods</h3>
                                    <div className="space-y-3">
                                      {planData.assessmentMethods.map((method: any, index: number) => (
                                        <div key={index} className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-2">{method.method || method.title || `Method ${index + 1}`}</h4>
                                          <p className="text-gray-400 text-sm">{method.description || 'No description available'}</p>
                                        </div>
                                      ))}
                                    </div>
                                  </div>
                                  ...
                                  <div className="mb-6">
                                    ...
                                    <div className="grid md:grid-cols-3 gap-4">
                                      {planData.proTips.learning && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-3">Learning Tips</h4>
                                      ...
                                      {planData.proTips.motivation && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-3">Motivation</h4>
                                          <ul className="space-y-1 text-sm text-gray-300">
                                      ...
                                      {planData.proTips.troubleshooting && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-3">Troubleshooting</h4>
                                          <ul className="space-y-1 text-sm text-gray-300">
                                  ...
                                  <div className="mb-6">
                                    ...
                                    <div className="grid md:grid-cols-3 gap-4">
                                      {planData.nextSteps.careerPaths && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-3">Career Paths</h4>
                                          <ul className="space-y-1 text-sm text-gray-300">
                                      ...
                                      {planData.nextSteps.communities && (
                                        <div className="bg-gray-700/30 rounded-lg p-4">
                                          <h4 className="font-semibold text-white mb-3">Communities</h4>
                                          <ul className="space-y-1 text-sm text-gray-300">
                                ...
                                {!planData.phases && !planData.overview && (
                                  <div className="bg-gray-700/30 rounded-lg p-4">
                                    <h3 className="text-xl font-bold text-white mb-4">Learning Plan Content</h3>
